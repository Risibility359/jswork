...
window.onload = function () {
    ...
    for (var i = 0, j = radios.length; i < j; j++) {
        radios[i].onclick = function () {
            ...
            document.getElementsById("JMS_main").style.width = value * 40 + 180 +60 + "px";
        }
    }
    init(10,10);
};

function init(rowCount, colCount,minlandMineCoun,maxlandMineCount) {
    var doc = document,
        landMineCountElement = doc.getElementsById("landMineCount"),
        timeShow = doc.getElementsById("costTime"),
        beginButton = doc.getElementsById("begin");
if (jms != null) {
...
jms.endCallBack = function (status) {
    clearInterval(timeHandle);
    document.getElementsById("begin").disable=""
    if(status){
        let name = document.getElementsById("name").innerText
        let score = document.getElementsById("costTime").innerText
...
beginButton.onclick = function () {
    ...
    timeHandle = setInterval(function () {
        timeShow.innerHTML = parseInt((new Data()- jms.beginTime) / 1000);
    },1000);
  };
}